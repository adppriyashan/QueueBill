...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
    <div class="card-header bg-white border-bottom p-4">
        ...
        <p class="text-secondary mb-0 fs-7">Browse generated cycle invoices, versions, simulated Google Drive uploads, and post-billing adjustments.</p>
    </div>
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        <tr>
                            ...
                            <td>
                                <div>
                                    <a href="{{ route('companies.show', $invoice->company) }}" class="fw-semibold text-secondary text-decoration-none hover-primary">{{ $invoice->company->name }}</a>
                                </div>
                            </td>
                            <td>
                                @if($invoice->recurringService)
                                    <a href="{{ route('services.show', $invoice->recurringService) }}" class="text-primary text-decoration-none hover-dark fs-7">{{ $invoice->recurringService->name }}</a>
                                @else
                            ...
                            <td>
                                @if($invoice->uploaded_to_drive_at)
                                    <span class="badge-premium bg-success-soft text-success text-truncate d-inline-block" style="max-width: 140px;" title="Google Drive: {{ $invoice->google_drive_path }}">
                            ...
                            <td class="text-end">
                                <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-light btn-sm border" title="Review Invoice Details"><i class="far fa-eye me-1"></i>View / Revise</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                <div class="text-muted mb-3">
                                    ...
                                    <p class="fs-7">Billing cycles occur automatically at midnight, or can be triggered manually from the Sandbox.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
