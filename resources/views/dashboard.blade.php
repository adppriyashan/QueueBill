...
@section('content')
    <!-- Welcome Banner -->
    <div class="card mb-4 border-0 shadow-sm rounded-4 text-white overflow-hidden animate-fade-in stagger-1"
        style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%);">
        <div
            class="card-body p-4 p-md-5 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-4">
            <div>
                <h2 class="fw-bold tracking-tight mb-2 text-white">Welcome to QueueBill Console</h2>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
                <div class="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('services.index') }}"
                        class="fs-7 text-primary text-decoration-none hover-primary">View All</a>
                </div>
                                        ...
                                        <td>
                                            <div>
                                                <a href="{{ route('services.show', $service) }}"
                                                    class="fw-bold text-dark text-decoration-none hover-primary">{{ $service->name }}</a>
                                                <span class="text-muted d-block fs-8">{{ $service->company->name }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <span
                                                class="fw-semibold text-dark">{{ $service->next_billing_date->format('M d, Y') }}</span>
                                        </td>
                                        <td>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
                <div class="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('invoices.index') }}"
                        class="fs-7 text-primary text-decoration-none hover-primary">View All Ledger</a>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-premium mb-0 align-middle fs-7">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <span
                                                    class="text-muted d-block fs-8">{{ $invoice->issue_date->format('M d, Y') }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="{{ route('companies.show', $invoice->company) }}"
                                                class="text-secondary text-decoration-none hover-primary">{{ $invoice->company->name }}</a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if($invoice->version > 1)
                                                <span
                                                    class="badge bg-warning text-dark font-weight-bold">v{{ $invoice->version }}</span>
                                            @else
                                        ...
                                        <td>
                                            @if($invoice->uploaded_to_drive_at)
                                                <span class="badge-premium bg-success-soft text-success"><i
                                                        class="fab fa-google-drive me-1"></i>Saved</span>
                                            @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4 text-muted fs-7">No statement ledger generated
                                            yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
