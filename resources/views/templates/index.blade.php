        ...
        <div class="card-body p-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                <div>
                    ...
                    <p class="text-secondary mb-0 fs-7">Configure separate branding rules, layouts, and sender overrides for
                        different lines of business.</p>
                </div>
                <a href="{{ route('templates.create') }}" class="btn btn-primary btn-sm d-inline-flex align-items-center">
                    ...
                    <tbody>
                        @forelse($templates as $template)
                            <tr>
                                ...
                                <td>
                                    <code
                                        class="fs-7 bg-light text-primary px-2 py-1 rounded">/invoices/templates/{{ $template->slug }}</code>
                                </td>
                                <td>
                                    @if($template->sender_email)
                                        <span class="fw-medium text-dark"><i
                                                class="far fa-envelope text-muted me-1"></i>{{ $template->sender_email }}</span>
                                    @else
                                        <span class="text-muted fs-8 font-italic"><i class="fas fa-cog text-muted me-1"></i>System
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex gap-1">
                                        <a href="{{ route('templates.preview', $template->slug) }}" target="_blank"
                                            class="btn btn-sm bg-success-soft text-success" title="Live Dynamic Demo">
                                            <i class="fas fa-external-link-alt"></i>
                                        </a>
                                        <a href="{{ route('templates.edit', $template) }}"
                                            class="btn btn-sm bg-warning-soft text-warning" title="Edit Configuration">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('templates.destroy', $template) }}" method="POST"
                                            class="d-inline-block"
                                            onsubmit="return confirm('Are you sure you want to deactivate/delete this template?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="text-muted mb-3">
                                        ...
                                        <p class="fs-7">Create templates to override sender profiles and custom layout branding
                                            rules.</p>
                                    </div>
