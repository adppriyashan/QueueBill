            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('templates.update', $template) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $template->title) }}" required placeholder="e.g. Web Hosting Corporate Template" autocomplete="off">
                        @error('title')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $template->slug) }}" placeholder="e.g. web-hosting-corporate" readonly>
                        </div>
                        @error('slug')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        <div class="form-text text-muted fs-8">Auto-generated alphanumeric parameter via URLs. Changing this updates dynamic config rendering pathways.</div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" class="form-control @error('sender_email') is-invalid @enderror" id="sender_email" name="sender_email" value="{{ old('sender_email', $template->sender_email) }}" placeholder="e.g. cloud-billing@corporate.com">
                        @error('sender_email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="active" {{ old('status', $template->status) === 'active' ? 'selected' : '' }}>Active (Available for scheduling)</option>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-4">
                        ...
                        <button type="submit" class="btn btn-primary">Update Structure Template</button>
                    </div>
                </form>
            </div>
        </div>
