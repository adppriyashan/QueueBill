    ...
    <div class="card mb-4 border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <div>
                ...
                <p class="text-secondary mb-0 fs-7">Manage system-wide alignment rules, defaults, and multi-tenant
                    parameters.</p>
            </div>
        </div>
    </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <p class="text-secondary mb-0 fs-8">Configure your company identity, addresses, and system-wide
                                default currency symbol.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('settings.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <select id="currency" name="currency"
                                    ...
                                    <option value="$" {{ old('currency', $user->currency) === '$' ? 'selected' : '' }}>$ -
                                        Dollar (USD, CAD, AUD)</option>
                                    <option value="€" {{ old('currency', $user->currency) === '€' ? 'selected' : '' }}>€ -
                                        Euro (EUR)</option>
                                    <option value="£" {{ old('currency', $user->currency) === '£' ? 'selected' : '' }}>£ -
                                        Pound (GBP)</option>
                                    <option value="¥" {{ old('currency', $user->currency) === '¥' ? 'selected' : '' }}>¥ -
                                        Yen/Yuan (JPY, CNY)</option>
                                    <option value="Rs." {{ old('currency', $user->currency) === 'Rs.' ? 'selected' : '' }}>Rs.
                                        - Rupee (INR, PKR)</option>
                                    <option value="LKR" {{ old('currency', $user->currency) === 'LKR' ? 'selected' : '' }}>LKR
                                        - Sri Lankan Rupee (LKR)</option>
                                    <option value="රු." {{ old('currency', $user->currency) === 'රු.' ? 'selected' : '' }}>රු.
                                        - Sri Lankan Rupee (රු.)</option>
                                    <option value="RM" {{ old('currency', $user->currency) === 'RM' ? 'selected' : '' }}>RM -
                                        Ringgit (MYR)</option>
                                    <option value="AED" {{ old('currency', $user->currency) === 'AED' ? 'selected' : '' }}>AED
                                        - Dirham (AED)</option>
                                    <option value="CHF" {{ old('currency', $user->currency) === 'CHF' ? 'selected' : '' }}>CHF
                                        - Franc (CHF)</option>
                                </select>
                            </div>
                            @error('currency')
                        ...
                        <div class="mb-4">
                            <label for="company_name" class="form-label text-secondary fw-semibold fs-7">Billed From:
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="company_name" name="company_name"
                                    ...
                                    placeholder="e.g. {{ env('APP_NAME', 'QueueBill') }}">
                            </div>
                            @error('company_name')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-4">
                            <label for="company_address" class="form-label text-secondary fw-semibold fs-7">Billed From:
                                Company Address <span class="text-danger">*</span></label>
                            <textarea id="company_address" name="company_address"
                                class="form-control @error('company_address') is-invalid @enderror" rows="3" required
                ...
                <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <p class="text-secondary mb-0 fs-8">Manage your account profile details and security
                                credentials.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('settings.profile.update') }}" method="POST">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-12 col-md-6">
                                <label for="email" class="form-label text-secondary fw-semibold fs-7 text-muted">Email
                                    Address</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-light text-muted"><i
                                            class="fas fa-envelope"></i></span>
                                    <input type="email" id="email" name="email" class="form-control bg-light text-muted"
                                        value="{{ $user->email }}" readonly disabled>
                                </div>
                                <div class="form-text text-muted fs-8"><i class="fas fa-info-circle me-1"></i> Email address
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12 col-md-6">
                                <label for="password" class="form-label text-secondary fw-semibold fs-7">New
                                    Password</label>
                                <div class="input-group">
                            ...
                            <div class="col-12 col-md-6">
                                <label for="password_confirmation"
                                    class="form-label text-secondary fw-semibold fs-7">Confirm Password</label>
                                <div class="input-group">
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                ...
                <div class="card-body p-4">
                    @if($user->google_access_token)
                        <div class="alert alert-success d-flex align-items-center mb-3 fs-7" role="alert"
                            style="background-color: rgba(25, 135, 84, 0.1); color: #198754; border: 0;">
                        ...
                        <form action="{{ route('auth.google.disconnect') }}" method="POST"
                            onsubmit="return confirm('Disconnect Google Drive?')">
                            @csrf
                    ...
                    @else
                        <p class="fs-7 text-secondary mb-3">Sync generated billing invoices directly to your personal or team
                            Google Drive paths automatically.</p>
