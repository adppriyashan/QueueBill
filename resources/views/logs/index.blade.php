...
@section('content')
    ...
    <div class="card mb-4 border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            ...
            <p class="text-secondary mb-0 fs-7">Review delivery outcomes, resend tracking logs, and simulated Google Drive
                storage uploads generated via automation schedules or retroactive revisions.</p>
        </div>
    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-premium mb-0 align-middle fs-7">
                            ...
                            <tbody>
                                @forelse($emailLogs as $log)
                                    <tr>
                                        <td>
                                            <div>
                                                <span
                                                    class="fw-bold text-dark d-block fs-8">{{ $log->invoice?->company?->name ?? 'Deleted Client' }}</span>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4 text-muted fs-8">No email broadcasts tracked in
                                            audit history.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="table-responsive">
                        <table class="table table-premium mb-0 align-middle fs-7">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <span
                                                    class="text-muted d-block fs-9">{{ $log->created_at->format('M d, Y H:i') }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            <code
                                                class="text-success fs-8"><i class="fas fa-folder me-1"></i>{{ Str::limit($log->google_drive_path, 22) }}</code>
                                        </td>
                                        <td>
                                            <span class="text-secondary fs-8"
                                                title="{{ $log->details }}">{{ Str::limit($log->details, 22) }}</span>
                                        </td>
                                        <td>
                                            <span class="badge bg-success-soft text-success"><i
                                                    class="fas fa-check me-1"></i>{{ strtoupper($log->status) }}</span>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4 text-muted fs-8">No Google Drive sync actions
                                            logged.</td>
                                    </tr>
                                @endforelse
                            </tbody>
