...
@section('title', 'Update Subscription Service - QueueBill')
@section('page_title', 'Edit Billing Schedule')
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
            <div class="card-header bg-white border-bottom p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="fw-bold mb-0">Update Subscription: {{ $service->name }}</h5>
                        <p class="text-secondary mb-0 fs-7">Modify billing cycles, templates, prices, scopes, and Google Drive upload paths.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('services.update', $service) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('invoice_structure_template_id') is-invalid @enderror" id="invoice_structure_template_id" name="invoice_structure_template_id" required>
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ old('invoice_structure_template_id', $service->invoice_structure_template_id) == $template->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $service->name) }}" required placeholder="e.g. SaaS Cloud Hosting Enterprise Suite">
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-12 col-sm-6">
                            ...
                            <input type="date" class="form-control @error('from_date') is-invalid @enderror" id="from_date" name="from_date" value="{{ old('from_date', $service->from_date->format('Y-m-d')) }}" required>
                            @error('from_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12 col-sm-6">
                            ...
                            <input type="date" class="form-control @error('to_date') is-invalid @enderror" id="to_date" name="to_date" value="{{ old('to_date', $service->to_date->format('Y-m-d')) }}" required>
                            @error('to_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-12 col-sm-6">
                            ...
                            <select class="form-select @error('recurring_cadence') is-invalid @enderror" id="recurring_cadence" name="recurring_cadence" required>
                                <option value="month" {{ old('recurring_cadence', $service->recurring_cadence) === 'month' ? 'selected' : '' }}>Month</option>
                                <option value="6_months" {{ old('recurring_cadence', $service->recurring_cadence) === '6_months' ? 'selected' : '' }}>6 Months</option>
                                <option value="year" {{ old('recurring_cadence', $service->recurring_cadence) === 'year' ? 'selected' : '' }}>Year</option>
                            </select>
                        ...
                        <div class="col-12 col-sm-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control @error('base_cost') is-invalid @enderror" id="base_cost" name="base_cost" value="{{ old('base_cost', $service->base_cost) }}" required placeholder="0.00">
                            </div>
                            @error('base_cost')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control border-start-0 @error('google_drive_path') is-invalid @enderror" id="google_drive_path" name="google_drive_path" value="{{ old('google_drive_path', $service->google_drive_path) }}" placeholder="e.g. /QueueBill/Invoices/AcmeCorp">
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="active" {{ old('status', $service->status) === 'active' ? 'selected' : '' }}>Active (Cycle runner active)</option>
                            <option value="inactive" {{ old('status', $service->status) === 'inactive' ? 'selected' : '' }}>Inactive (Cycle suspended)</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-4">
                        <a href="{{ route('services.show', $service) }}" class="btn btn-light border">Cancel</a>
                        <button type="submit" class="btn btn-primary">Update Billing Contract</button>
                    </div>
                </form>
            </div>
        </div>
