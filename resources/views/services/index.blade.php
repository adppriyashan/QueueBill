    ...
    <div class="card-body p-4">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            <div>
                ...
                <p class="text-secondary mb-0 fs-7">Connect buyers into pricing schedules, intervals, and Google Drive upload paths.</p>
            </div>
            <a href="{{ route('services.create') }}" class="btn btn-primary d-inline-flex align-items-center">
                ...
                <tbody>
                    @forelse($services as $service)
                        <tr>
                            <td>
                                <div>
                                    <a href="{{ route('companies.show', $service->company) }}" class="fw-bold text-dark text-decoration-none hover-primary">{{ $service->company->name }}</a>
                                    <span class="text-muted d-block fs-8">{{ $service->company->email }}</span>
                                </div>
                            </td>
                            <td>
                                <div>
                                    <a href="{{ route('services.show', $service) }}" class="fw-semibold text-primary text-decoration-none hover-dark d-block">{{ $service->name }}</a>
                                    <span class="text-secondary fs-8">Template: {{ $service->invoiceStructureTemplate->title }}</span>
                                </div>
                            </td>
                            <td>
                                <div class="fs-8">
                                    <span class="d-block"><strong>From:</strong> {{ $service->from_date->format('M d, Y') }}</span>
                                    <span class="d-block"><strong>To:</strong> {{ $service->to_date->format('M d, Y') }}</span>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="text-dark fw-semibold">{{ $service->next_billing_date->format('M d, Y') }}</span>
                                </div>
                            </td>
                            <td>
                                @if($service->google_drive_path)
                                    <code class="fs-8 bg-light text-success px-2 py-1 rounded" title="{{ $service->google_drive_path }}"><i class="fab fa-google-drive me-1"></i>{{ Str::limit($service->google_drive_path, 20) }}</code>
                                @else
